...
(define-key edit-server-text-mode-map (kbd "C-c C-c") 'edit-server-abort)
(define-key edit-server-text-mode-map (kbd "C-x C-c") 'edit-server-done)
...
(defun edit-server-create-edit-buffer(proc)
  ...
  (let ((buffer (generate-new-buffer (if edit-server-url
    ...
    (with-current-buffer buffer
      ...
      (write-file (concat tmp-file-dir tmp-filename))
